...
div()
  ...
  v-card
    div
      v-btn(router,fab,absolute,top,right,dark,class="green", :to="{name: 'mailListCreate', params: {resource}}",v-if="options.create !== false")
        v-icon add
      ...
      template(slot='items', scope='props')
        tr
          ...
          td(v-if='actions !== false', width='160')
            ...
            v-btn(v-if="options.edit !== false",dark,primary,fab,small, :to="{name: 'mailListEdit', params: {resource,id:props.item.address}}")
              v-icon edit
...
export default {
  ...
  methods: {
    ...
    getColumnData (row, field) {
      ...
      if (tag) {
        try {
          value = `<${tag} href="/#/maillists/${row.address}/members" class="btn btn--raised btn--small theme--dark warning" controls> ${value}  </${tag}>`
        } catch (error) {
          value = `<${tag} href="/#/maillists/${row.address}/members" class="btn btn--raised btn--small theme--dark warning" controls> ${value}  </${tag}>`
        }
      }
    ...
    fetchGrid () {
      this.columns = [
        ...
        {text: 'Description', type: 'text', value: 'description'},
        {text: 'Members Count', type: 'button', value: 'members_count'},
        {text: 'Access Level', type: 'radio', value: 'access_level'}
      ];
