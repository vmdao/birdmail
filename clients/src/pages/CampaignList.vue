...
div()
  ...
  v-card
    div
      v-btn(router,fab,absolute,top,right,dark,class="green", :to="{name: 'campaignCreate', params: {resource}}",v-if="options.create !== false")
        v-icon add
      ...
      template(slot='items', scope='props')
        tr
          ...
          td(v-if='actions !== false', width='180')
            ...
            v-btn(v-if="options.edit !== false",dark,primary,fab,small, :to="{name: 'campaignEdit', params: {resource,id:props.item.id}}")
              v-icon edit
  ...
  methods: {
    ...
    fetchGrid () {
      this.columns = [
        ...
        {text: 'Send', type: 'text', value: 'submitted_count', align: 'left'},
        {text: 'Opend', type: 'text', value: 'opened_count', align: 'left'},
    ...
    fetchData () {
      ...
      this.$http.get(`${this.resource}`, {params: this.$route.query}).then(({ data }) => {
        console.log(data, data.total_count, 'data')
        this.pagination.totalItems = data.total_count || 0;
